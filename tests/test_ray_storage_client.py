...
def ray_storage_client(ray_setup):
    client = RayStorageClient.remote()
    yield client
    try:
...
def test_ray_storage_put_get_gpu(ray_storage_client):
    ...
    values = [
        torch.randn(100, 200).cuda(),
        torch.randn(50, 150).cuda()
    ]
...
def test_nixl_vs_object_store_performance_multi_tensors(ray_storage_client, tensor_shape: Tuple[int, int], dtype: torch.dtype):
    device = "cuda"
    ...
    print(f"NIXL Time: {nixl_time:.4f}s | Object Store Time: {obj_time:.4f}s | Speedup: {speedup:.2f}x | latency_improvement: {latency_improvement:.2f}%")
